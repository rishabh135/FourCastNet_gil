...
def setup(params):
	...
	if params.log_to_screen:
		logging.info(
			f"Loading trained model checkpoint from {params['best_checkpoint_path']}"
		)

	in_channels = np.array(params.in_channels)
	out_channels = np.array(
		params.in_channels
	)  # for the backbone model, will be reset later
	...
	if params.log_to_screen:
		...
		logging.info(f"Validation data from {files_paths[yr]}")
	...
	if (params.log_to_screen):
		...
		logging.info(f"Validation data from {precip_paths[0]}")
...
def autoregressive_inference(params, ic, valid_data_full, valid_data_tp_full, model_wind, model):
	...
	valid_loss = torch.zeros((prediction_length, n_out_channels)).to(
		device, dtype=torch.float
	)
	acc = torch.zeros((prediction_length, n_out_channels)).to(device, dtype=torch.float)
	acc_unweighted = torch.zeros((prediction_length, n_out_channels)).to(
		device, dtype=torch.float
	)
	tqe = torch.zeros((prediction_length, n_out_channels)).to(device, dtype=torch.float)

	# wind seqs
	seq_real = torch.zeros(
	...
	valid_data_tp = valid_data_tp_full[
		ic : (ic + prediction_length * dt) : dt, 0:720
	].reshape(
		len_ic, n_out_channels, 720, img_shape_y
	)  # extract valid data from first year
	# log normalize
	...
	m = torch.as_tensor(np.load(params.time_means_path_tp)[0][out_channels])[
		:, 0:img_shape_x
	]  # climatology
	...
	if orography:
		orog = torch.as_tensor(
			np.expand_dims(
				np.expand_dims(h5py.File(orography_path, "r")["orog"][0:720], axis=0),
				axis=0,
			)
		).to(device, dtype=torch.float)
		logging.info(f"orography loaded; shape:{orog.shape}")
	...
	with torch.no_grad():
		for i in range(valid_data.shape[0]):
			...
			if i < prediction_length - 1:  # not on the last step
				...
				seq_real_tp[i + 1] = unlog_tp_torch(
					future_tp
				)  # which is the i+1th validation data
				# collect history
			...
			if params.log_to_screen:
				logging.info(
					f"Timestep {i} of {prediction_length}. TP RMS Error: {valid_loss[i, 0]}, ACC: {acc[i, 0]}"
				)
	...
	tqe = tqe.cpu().numpy()
	return (
		np.expand_dims(seq_real_tp, 0),
		np.expand_dims(seq_pred_tp, 0),
...
if __name__ == "__main__":
	...
	logging_utils.log_to_file(
		logger_name=None, log_filename=os.path.join(expDir, "./precipitation_inference_out.log")
	)
	...
	logging.info(f"Inference for {n_ics} initial conditions")
	try:
	...
	for i, ic in enumerate(ics):
		t1 = time.time()
		logging.info(f"Initial condition {i + 1} of {n_ics}")
		sr, sp, vl, a, au, tq = autoregressive_inference(
			params, ic, valid_data_full, valid_data_tp_full, model_wind, model
		)
		if i == 0:
			seq_real = sr
			seq_pred = sp
	...
	if params.log_to_screen:
		logging.info(
			f"Saving files at {os.path.join(params['experiment_dir'], 'autoregressive_predictions' + autoregressive_inference_filetag + '.h5')}"
		)
	with h5py.File(
		os.path.join(
