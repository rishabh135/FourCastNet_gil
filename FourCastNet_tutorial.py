...
# Run interactively in the backend:
# $ sinteractive -N1 -n4 --gres=gpu:1 -A standby -t 4:00:00
# Run with Run_FourCastNet_tutorial.sh
# $ ./Run_FourCastNet_tutorial.sh
...
def autoregressive_inference(
    ...
    if params.masked_acc:
        maskarray =             torch.as_tensor(np.load(params.maskpath)[0:720]).to(device,
                dtype=torch.float)
    ...
    with torch.no_grad():
        ...
        # Compute metrics
            ...
            if params.interp > 0:
                ...
                acc_coarse_unweighted[i] =                     unweighted_acc_torch_channels(pred - clim_coarse,
                        tar - clim_coarse)
...
def main(args):
    ...
    if args.exp_dir is not None:
        assert args.weights is not None,             'Must set --weights argument if using --exp_dir'
    else:
        assert args.weights is None,             'Cannot use --weights argument without also using --exp_dir'
...
parser.add_argument('--exp_dir', default=f"/scratch/gilbreth/{username}/FourCastNet_tutorial_expts/", type=str, help='Path to store inference outputs; must also set --weights arg')
